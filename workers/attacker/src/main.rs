...
struct Args {
    ...
    #[arg(short, long, default_value = "attacker-1")]
    worker_id: String,
...
async fn main() -> Result<()> {
    ...
    let args = Args::parse();
    info!("Starting attack worker {}", args.worker_id);
...
async fn register_worker(
    ...
    let request = RegisterRequest {
        ...
        capabilities: Some(WorkerCapabilities {
            job_types: vec!["ATTACK".to_string()],
...
async fn get_job(
    ...
    let request = GetJobRequest {
        worker_id: worker_id.to_string(),
        session_token: session_token.to_string(),
        preferred_types: vec!["ATTACK".to_string()],
    };
...
async fn execute_job(
    ...
    info!("Executing attack job {}", job.job_id);
    
    // Execute the attack
    let start_time = std::time::Instant::now();
    let result = attacker::execute_attack(&job).await?;
